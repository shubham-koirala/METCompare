...
########################################    
# (d): number of DESeq2-called DE genes retained byDEBoost
#########################################   
...
df <- foreach(cancer.type = names(DE.rs.list),.combine='rbind') %do% {
    ...
    r.cnt         <- after.cnt 
    e.cnt         <- before.cnt - after.cnt
    data.frame(cancer.type = cancer.type, gene.type = c('excluded','retained'),value=c(e.cnt,r.cnt))
}
...
##################
# (a): heatmap to the expression pattern of up regulated genes retained by DEBoost
#################
