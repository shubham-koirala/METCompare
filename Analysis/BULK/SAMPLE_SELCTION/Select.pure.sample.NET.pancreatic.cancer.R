...
met.sample   <- GEP.net.meta$Run[GEP.net.meta$origin == 'pancreas' & GEP.net.meta$type == 'liver metastasis' ] %>% as.character()
pri.sample   <- GEP.net.meta$Run[GEP.net.meta$origin == 'pancreas' & GEP.net.meta$type == 'primary' ] %>% as.character()
...
pure.PRI.NET.pancreatic.cancer.sample    <- names(expr.cor)[(expr.cor > cor.cut.off)  & (cell.line.rank > rank.cut.off) ]
...
pure.MET.NET.pancreatic.cancer.sample    <- names(expr.cor)[(expr.cor > cor.cut.off)  & (cell.line.rank > rank.cut.off) ]

MET500.best.cell.line <- rs.MET500$best.cell.line
TCGA.best.cell.line   <- rs.TCGA$best.cell.line


save(file = 'client-side/output/Select.pure.sample.NET.pancreatic.cancer.R.output/Select.pure.sample.NET.pancreatic.cancer.RData',list=c('pure.PRI.NET.pancreatic.cancer.sample','pure.MET.NET.pancreatic.cancer.sample','MET500.best.cell.line','TCGA.best.cell.line'))
