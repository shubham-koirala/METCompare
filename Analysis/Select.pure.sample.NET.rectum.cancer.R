...
met.sample   <- GEP.net.meta$Run[GEP.net.meta$origin == 'rectum' & GEP.net.meta$type == 'liver metastasis' ] %>% as.character()
pri.sample   <- GEP.net.meta$Run[GEP.net.meta$origin == 'rectum' & GEP.net.meta$type == 'primary' ] %>% as.character()
...
pure.PRI.NET.rectum.cancer.sample    <- names(expr.cor)[(expr.cor > cor.cut.off)  & (cell.line.rank > rank.cut.off) ]
...
pure.MET.NET.rectum.cancer.sample    <- names(expr.cor)[(expr.cor > cor.cut.off)  & (cell.line.rank > rank.cut.off) ]


save(file = 'client-side/output/Select.pure.sample.NET.rectum.cancer.R.output/Select.pure.sample.NET.rectum.cancer.RData',list=c('pure.PRI.NET.rectum.cancer.sample','pure.MET.NET.rectum.cancer.sample'))
